const fs = global.nodemodule["fs-extra"];
const path = global.nodemodule["path"];

module.exports.config = {
  name: "autoreplybot",
  version: "6.0.2",
  hasPermssion: 0,
  credits: "𝐒𝐡𝐚𝐡𝐚𝐝𝐚𝐭 𝐈𝐬𝐥𝐚𝐦",
  description: "Auto-response bot with specified triggers",
  commandCategory: "No Prefix",
  usages: "[any trigger]",
  cooldowns: 3,
};

module.exports.handleEvent = async function ({ api, event, Users }) {
  const { threadID, messageID, senderID, body } = event;
  if (!body) return; 
  const name = await Users.getNameUser(senderID);
  const msg = body.toLowerCase().trim();

  const responses = {
    "miss you": "অরেক বেডারে Miss না করে xan মেয়ে হলে বস অভি রে বিয়ে করো😶👻😘𝐅𝐀𝐂𝐄𝐁𝐎𝐎𝐊 𝐈𝐃 𝐋𝐈𝐍𝐊 🌻:- https://www.facebook.com/100078223341664",
    "kiss de": "কিস দিস না তোর মুখে দূর গন্ধ কয়দিন ধরে দাঁত ব্রাশ করিস নাই🤬",
    "👍": "সর এখান থেকে লাইকার আবাল..!লাইক না দিয়ে ভালোভাবে কথা বল😡🐸🤣👍⛏️",
    "রাব্বি": "শুনলাম তোমাদের রাব্বি ভাইয়া গার্লফ্রেন্ড নিয়ে চিপায় ধরা পড়ছে সত্যি নাকি😩🤣",
    "hi": "এত হাই-হ্যালো কর ক্যান প্রিও. সালাম দিতে শিখো.!😜🫵",
    "i miss you": "SAME TO You মেয়ে হলে এডমিন রাব্বি কে নক দেও 😊",
    "ki kro": "Khud k0o KYa LeGend SmJhTi Hai তোমাকে মিস করতেছি জান 😂",
    "good morning": "GOOD MORNING আলহামদুলিল্লাহ আরো একটি সকাল পেলাম এখন দাত ব্রাশ করে খেয়ে নেও😚",
    "bal": "~ পচা কথা বলে না জান মানুষ লুচ্চা বলবে?? 🤖",
    "ovi": "উনি এখন  বিজি আছে কি বলবেন আমাকে বলতে পারেন..!😘জরুরি হলে ইনবক্স নক দেন𝐅𝐀𝐂𝐄𝐁𝐎𝐎𝐊 𝐈𝐃 𝐋𝐈𝐍𝐊 🌻:- https://www.facebook.com/100078223341664",
    "all ceo": "‎[all ceo:☞ ☜Box2 ceo rakibul hasan rakib/❤️‍🩹 box4 Faija ma'am 🌹❤️‍🩹 box 1 Sakib🌹 sumu rabby mithila❤️‍🩹🙊😘",
    "admin": "He is MBH ovi তাকে সবাই কিছু স্বপ্ন কিছু আসা ফ্যামেলি বক্স creator হিসেবে চিনে😘☺️",
    "bot tmr boss er name ki": "আমার বস অভি তিনি হলেন কিছু স্বপ্ন কিছু আসা ফ্যামেলি বক্সের creator 😍.",
    "chup": "তুই চুপ চুপ কর পাগল ছাগল তোর কথা শুনার টাইম নাই🤣😩😒",
    "assalamualaikum": "❤️‍🩹😍 Walaikumus Salam o rohmatullah ❤️‍🩹কেমন আছেন 💖",
    "@প্রেয়সী স্নিগ্ধা": "তোর সাহস তো কম না অভির হবু বউ কে ম্যানশন দিস আর ভাবি বল ভাবি🙊🤫",
    "kiss me": "তুমি পঁচা তোমাকে কিস দিবো না 🤭",
    "thanks": "এতো ধন্যবাদ না দিয়ে আমার বস ovi রে তোর গার্লফ্রেন্ড টা দিয়ে দে..!🐸🥵",
    "i love you": "মেয়ে হলে আমার বস ovi এর ইনবক্সে এখুনি গুঁতা দিন🫢😻",
    "by": "কিরে তুই কই যাস কোন লাঙ্গের সাথে চিপায় যাবি..!🌚🌶️",
    "@অভিমানী মন আমার‎": "ম্যানশন দিয়ে লাভ নাই তানজিনা এখন বস অভির সাথে বিজি আছে🌶️",
    "tanjina": "সর ডাকিস না..! তানজিনা বস অভির সাথে বিজি আছে..!🤐🙊",
    "@মায়াবী পরী": "দিন শেষে পরের বউ সুন্দর ম্যানশন দিয়ে লাভ নাই অন্যের বউ লাগে মনে তোর ভাবি🤧📢🥱",
    "bissti": "কি বেপার তুই দেখি চিপাবাজ বৃষ্টি কে ডাকতেছিস উনি নাই কি বলবি আমাকে বল..!",
    "@احسن حبیب": "দেখ পাগল রে আবার লারা দিছে😂🤧🥱..! সুন পাগল চেতান ভালো না লারা ডিস না...!😂🤐",
    "mithila": "কি হলো মিস্টেক করেছিস নাকি..!📢",
    "jannatul": "ভাগ্য করে একটা loyal বেডি পাইছিলাম, অন্য বেডা তো দূরে থাক আমাকেই পাত্তা দিচ্ছে না।...!😑😒🤧",
    "@স্বপ্নের ঠিকানা": "L.CEO মাহী সিঙ্গেল আছে. চাইলে প্রপোজ করতে পারো..💔😘📢",
    "ami tor boss": "হ্যা বস কেমন আছেন..?☺️",
    "ovir baccha": "অভির বাচ্চা তো তোমার গার্লফ্রেন্ডের পেটে..!!🌚⛏️",
    "@সুহা শিনী": "ম্যানশন দিয়ে লাভ নাই এটা অভির হবু বউ মানে তদের ভাবি লাগে🤐🤧..!", 
    "@Log Out": "উপস্থাপক কিউট এর ডিব্বা মানে ফাল্টিং এর রাজা মেয়েরা সাবধান..😂😂",
    "@Fabiha Nowshin": "📢 everyone নশিন আর নাই মানে গ্রুপে নাই চিপায় আটকে গেছে",
    "@MI S TY": "অযথাই ম্যানশন ডিস না ভাই পারলে একটা জামাই দে..😘🤧",
    "@لامیا اختر": "লুচ্ছাদের মত অন্যের gf রে ম্যানশন ডিস না পারলে নিজে একটা gf বানা..📢",
    "@Crush Mim": "তুই দয়াল বেডি মীম রে খুঁজিস কেনো প্রেম করবি নাকি😂🤧",
    "@Tasnia Rahman Tanha": "ম্যানশন ডিস না অভির সাথে মিটিং এ বিজি আছে..💖🤐",
    "@Sakib Sakib": "চাকিপ কান রে ম্যানশন ডিস না আপু বিশ্বাস এর সাথে চিপায় আছে😁😩",
    "@Sov Nil Akash": "মাস্টার মাইন্ড লুচ্চা আকাশ তাকে পেতে হলে চিপা চাপা মেয়েদের ইনবক্স এ খুজতে হবে..🥱🤐😁",
    "@RN 卝 Jenyツ": "বস অভির সাথে চিপায় আছে এখন বিরক্ত করিস না🥱🤐😩",
    "tmr name ki": "এত দিন থেকে গ্রুপে আছো নামে জানো না MY NAME IS ─꯭─⃝‌‌অভির পিচ্ছি বউ💖",
    "sakib": "তুই মরা মুরগি সাকিব কে ডাকিস কেন 🐔🐣🐥💔এন থেকে সর দুরে গিয়া মর😒",
    "রুল্স": "2️⃣ মেম্বার এর সাথে গালা গালি করে কথা বল্লে  কিক❌😒3️⃣গুটি বাজ ফাপর বাজ হলে কিক❗✅😾4️⃣active না থাকলে কিক❌😈5️⃣রুলস না মানলে কিক❌👿6️⃣ ১৮+ কোনো কথা পিক ভিডিও  দিলে কিক❌9️⃣ বিনা অনুমতি ছাড়া কোনো ছেলে / মেয়ে করো ইনবক্সে নক দিলে নোটিশ ছাড়া কিক❌👿1️⃣1️⃣ বাংলাদেশ টাইম রাত 8:50টা থেকে ১০:০০ টা পর্যন্ত এস এস তখন কলে না আসলে কিক❌👿☑️প্রতিদিন কলে ২/৩ ঘন্টা সময় দিতে হবে ❗ধন্যবাদ 💔 কারণ ছাড়া ss টাইম মিস করলে কিক😡😡❤️‍🩹কলে কোনো সমস্যার কারনে না আসতে পারলে,গ্রুপ এডমিন কে বলতে হবে, যে কলে আসতে পারবো না,, ✅😘ধন্যবাদ কলিজার ভাই & আপু রা.!🥱🌝🌚",
    "bal": "রাগ করে না সোনা পাখি 🥰",
    "নোটিশ": "আমাদের ss time রাত ৯-১০ এই টাইম সবাইকে কল থাকতে হবে বাধ্যতা মূলক। কারণ ছাড়া ss টাইম মিস করলে কিক😡😡😈🥰",
    "ফাইজা": "খবরদার এই নাম ধরে ডাকবি না এটা বস অভির হবু বউ ..!🌚🤣",
    "bot i love you": "আমাকে না বলে। প্রেম করতে চাইলে Boss ovi এর ইনবক্সে গুতা দিন 😘",
    "@everyone": "📢@everyone চিপা থেকে বের হয় গ্রুপে আসো সময় দেও😈.!🌚",
    "হাসান": "হ্যাঁ সব কেমন আছেন আপনার ওই খানে উম্মাহ 😘😽🙈"
  };

  if (responses[msg]) {
    return api.sendMessage(responses[msg], threadID, messageID);
  }
};

module.exports.run = async function ({ api, event, args, Users }) {
  return this.handleEvent({ api, event, Users });
};
